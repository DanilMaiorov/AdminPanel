...
import { sortUsers } from './modules/sortUsers'; //импортируем через {} потому что экспортировали не по дефолту
import { searchUsers } from './modules/searchUsers'; //импортируем через {} потому что экспортировали не по дефолту
...
userService.getResponse('http://localhost:8080/users').then(data => { //вызовем метод getUsers(), который берет данные с сервера, юрл которого передан в userService и обработаем еще раз чтобы с данными можно было работать
